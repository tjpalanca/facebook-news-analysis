...
# Create database
fbpages.sqlite <- src_sqlite(path = "../dta/03-fbpages.sqlite",
                             create = TRUE)

```

We then parse through each of the data files and load them into the SQLite database.
...
walk(
  ...
  function(data_path) {
      ...
      function(i) {
        ...
        walk(
          c(
            ...
            "posts_comments_comments", "posts_comments_comments_likes"
          ),
          function(name) {
    ...
    # Rename file to indicate done
    file.rename(
      from = data_path,
      to = str_replace(data_path, "03-fbpage-", "03-fbpageDONE-")
    )
  }
)
...
# TODO: sqlite optimizations (indices, etc.)
