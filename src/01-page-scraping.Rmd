...
na_null <- function(obj) {
  # Helper function, returns NA of obj is null
  ...
  posts.ls %>% 
    ...
    mutate(
      ...
      data_frame(
        page_name    = .$page_name,
        post_id      = .$id, 
        post_message = .$message %>% na_null(), 
        post_timestamp_utc = .$post_timestamp_utc %>% na_null(),
        post_story   = .$story %>% na_null()
      )
    } -> posts.dt
  message(" Done!")
      ...
      map_df(
        function(attachments) {
          data_frame(
            ...
            attachment_title       = attachments$title %>% na_null(),
            attachment_type        = attachments$type %>% na_null(),
            attachment_target_url  = attachments$target$url %>% na_null(),
            attachment_media_url   = attachments$media$image$src %>% na_null()
          )
        }
      )->
...
rappler_posts.ls <-
  try({
    cachedGetFBPage(
      ...
      cache_interval              = "3 days",
      posts_per_page              = 5,
      num_comments                = 100, 
      num_attachments             = 100,
      num_reactions               = 1000,  
      num_comments_likes          = 500, 
      num_comments_comments       = 50,
      num_comments_comments_likes = 500,
      access_token                = fb.tkn
    )
  })

saveRDS(rappler_posts.ls, "../dta/03-rappler-posts.rds")

load("../bin/pushover_auth.rda")

pushover(
  title = "R Execution Completed",
  message = ifelse(class(rappler_posts.ls) == "try-error", rappler_posts.ls[[1]], "Success"),
  priority = 1,
