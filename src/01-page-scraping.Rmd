    ...
    map_df(
      function(attachments) {
        data_frame(
          ...
          attachment_title       = ifelse(
            is.null(attachments$title), NA,
            attachments$title
          ),
          attachment_type        = ifelse(
            is.null(attachments$type), NA,
            attachments$type
          ),
          attachment_target_url  = ifelse(
            is.null(attachments$target$url), NA,
...
cachedGetFBPage <- function(page_name, limit_timestamp, 
  ...
  if (file.exists("../cache/FBpage.rds")) {
    message("Cache exists: Checking if page name match")
    # Record cache
    cache <- readRDS("../cache/FBpage.rds")
  ...
  if (length(cache_intervals.ls) > 0) {
    for (interval in cache_intervals.ls) {
      ...
      # Cache
      saveRDS(posts.ls, "../cache/FBpage.rds")
    }
  }
...
In order to start collecting information, we 
...
```{r Rappler 2016 Posts, message = FALSE}

# Exhibit: Download all Rappler posts for 2016
...
rappler_posts.dt <-
  try({
    cachedGetFBPage(
      ...
      cache_interval              = "7 days",
      posts_per_page              = 5,
      num_comments                = 10, 
      num_attachments             = 10,
      num_reactions               = 100,  
      num_comments_likes          = 50, 
      num_comments_comments       = 10,
      num_comments_comments_likes = 50,
      access_token                = fb.tkn
    )
  })

while(class(rappler_posts.dt) == "try-error" & errors < 100) {
  
  # Increase error count if errored out
  errors <- errors + 1
  message(errors, " errors made. ", 100 - errors, " errors left.")
...
saveRDS(rappler_posts.dt, "../dta/03-rappler-posts.rds")
