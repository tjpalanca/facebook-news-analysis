...
cachedGetFBPage <- function(page_name, limit_timestamp, cache_interval, ...) {
  
  # Set cache intervals
  cache_intervals.ls <- 
    rev(seq(as.POSIXct(limit_timestamp, tz = "SGT"), Sys.time(), cache_interval))
  cache_intervals.ls <-
    cache_intervals.ls[2:length(cache_intervals.ls)]
  ...
  # Show cache intervals
  cat(paste0(format(cache_intervals.ls, "%Y-%m-%d %z"), collapse = "\n"), "\n")
  
  # Get information and cache
  for (interval in cache_intervals.ls) {
    cat(format(as.POSIXct(interval, origin = "1970-01-01"), "%Y-%m-%d %z"), "\n")
    # Get information
    posts.ls %<>% append(
      getFBPage(
        page_name       = page_name,
        next_link       = ifelse(length(posts.ls) == 0, NA, posts.ls[length(posts.ls)]$next_link),
