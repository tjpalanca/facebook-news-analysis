...
expandPaging <- function(object_ids, object_data, object_next) {
  ...
  pmap(
    ...
    function(object_id, object_data, object_next) {
      ...
      message("Expanding pagination for ", object_id, "...")
...
queryFBPosts <- function(num_comments, num_attachments, num_reactions) {
  ...
  # 
  # Args:
  #   num_comments: number of comment entities
  #   num_attachments: number of attachment entities
  #   num_reactions: number of recation entities
  #   
  # Returns:
...
getFBPage <- function(page_name = NULL, next_link = NULL, limit_posts = Inf, limit_timestamp = -Inf, 
...
tidyFBPageData <- function(posts.ls, access_token = fb.tkn) {
  ...
  message("Grabbing post attachments data...\n")
  posts.ls %>% 
  ...
  message("Grabbing post reactions data...\n")
  posts.ls %>% 
    select(id, reactions) %>% {
      expandPaging(
        object_ids  = .$id,
  ...
  message("Grabbing post comments raw data...\n")
  posts.ls %>% 
    select(id, comments) %>% {
      expandPaging(
        object_ids  = .$id,
  ...
  message("Grabbing post comments processed data...\n")
  posts_comments.ls %>% 
    map_df(
      function(comments) {
  ...
  message("Grabbing post comments reactions data...\n")
  posts_comments.ls %>% 
    map(
      function(comments) {
        list(
  ...
  message("Grabbing post comments replies raw data...\n")
  posts_comments.ls %>% 
    map(
      function(comments) {
        list(
  ...
  message("Grabbing post comments replies processed data...\n")
  posts_comments_comments.ls %>% 
    map_df(
      function(comments) {
        if (is.null(comments$id)) return(NULL)
  ...
  message("Grabbing post comments replies reactions data...\n")
  posts_comments_comments.ls %>% 
    map(
      function(comments) {
        list(
...
cachedGetFBPage <- function(page_name, limit_date, cache_interval, 
                            timezone = "Asia/Manila", ...) {
  # Grabs complete data from a Facebook Page as specified by a date limit. 
  # Unlike getFBPage, this method caches every cache_interval to hidden object 
  # .cache_fbpage so that the process can resume in cases of failure.
  # 
  # Args:
  #   page_name: Facebook Page ID of the page
  #   limit_date: string representing the date limit (earliest) in format YYYY-MM-DD
  #   cache_interval: in words, the interval at which it will cache (e.g. 1 day)
  #   timezone: The relevant timezone for the postings (defaults to Asia/Manila)
  ...
  cache_intervals.ls <- 
    seq(
      as.POSIXct(paste0(limit_date, " 00:00:00"), tz = timezone), 
      Sys.time(), 
      cache_interval
    ) %>%
    rev()
  ...
  if (length(cache_intervals.ls) > 0) {
    for (interval in cache_intervals.ls) {
      interval_text <- format(as.POSIXct(interval, origin = "1970-01-01"), "%Y-%m-%d %z")
        ...
        list(
          getFBPage(
            ...
            next_link       = ifelse(
              ...
              posts.ls[[length(posts.ls)]]$next_link
            ),
  ...
  for (index in 1:length(posts.ls)) {
    message("Expanding pages... ", round(index/length(posts.ls) * 100), "% complete.")
    # If posts_expanded does not exist, add it
    if (is.null(posts.ls[[index]]$posts_expanded)) {
      posts.ls[[index]]$posts_expanded <-
        posts.ls[[index]]$posts %>% tidyFBPageData()
    }
    # Cache
    .cache_fbpage <<- posts.ls
  }
  
  # Clean cache and return data
  message("Done! Cleaning cache and returning clean data..")
  rm(.cache_fbpage, envir = globalenv())
  posts.ls
}
