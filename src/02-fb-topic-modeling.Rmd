...
theme_set(
  theme_bw(base_family = "Roboto", base_size = 14) +
    theme(
      plot.title = element_text(face = "bold", size = 12, margin = margin(0, 0, 4, 0, "pt")),
...
topics_mapping.dt <- read_csv("../dta/lda-topics.csv", col_types = "ic")
    ...
    ggsave(
      plot = {
        topic_labelling_top30words.dt %>% 
          ggplot(aes(x = x, y = y)) +
          geom_text_repel(
            aes(label = term, color = factor(topic), size = phi_kw * p_w), 
            lineheight = 0.6, 
            segment.size = NA,
          ...
          geom_text_repel(
            ...
            aes(label = str_wrap(topic_name, 15), color = factor(topic)),
