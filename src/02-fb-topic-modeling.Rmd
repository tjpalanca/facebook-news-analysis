...
posts_classification.dt %>% 
  inner_join(
    posts.dt %>% 
      select(document = post_id, news_page = page_name, 
             attachment_title, post_timestamp_utc),
    by = "document"
  ) %>% 
  inner_join(topics_mapping.dt, by = "topic") %>% 
  filter(
    !topic_name %in% c("Filipino News", "Filipino Showbiz", 
  ...
  ungroup() %>%
  arrange(peak, week) %>%
  mutate(topic_name = factor(topic_name, levels = rev(unique(topic_name))),
...
bind_rows(
  ...
  inner_join(posts_time_trend.dt %>% select(topic_name, week, id), 
             by = c(".group" = "id")) ->
