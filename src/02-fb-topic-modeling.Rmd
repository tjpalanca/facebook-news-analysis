...
suppressPackageStartupMessages({
  ...
  library(hunspell)  # Spell checking, stemming, and others
})
...
posts_tokenized.dt %>% 
  ...
  mutate(
    tl_spellcheck = hunspell_check(word, "tl_PH"),
    en_spellcheck = hunspell_check(word, "en_US"),
    fail_spellcheck = !tl_spellcheck & !en_spellcheck
  ) 
  
  # For words that do not pass either spell check,
  # get list of suggested words
  mutate(
    tl_suggest = 
      map2(
        word, fail_spellcheck,
        ~ifelse(.y, hunspell_suggest(.x, "tl_PH"), NA)
      ),
    en_suggest = 
      map2(
        word, fail_spellcheck,
